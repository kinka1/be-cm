...
class PurchaseOrderController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $po = DB::transaction(function () use ($data) {
            $po = PurchaseOrder::create(['po_number' => 'PO-'.now()->format('YmdHis').'-'.random_int(1000,9999), 'supplier_id' => $data['supplier_id'] ?? null, 'employee_id' => $data['employee_id'] ?? null, 'order_date' => $data['order_date'], 'status' => 'draft', 'notes' => $data['notes'] ?? null]);
            $total = 0;
    ...
    public function update(Request $request, PurchaseOrder $purchaseOrder): JsonResponse
    {
        ...
        $purchaseOrder->update($request->validate(['supplier_id' => ['nullable','exists:suppliers,id'], 'employee_id' => ['nullable','exists:employees,id'], 'order_date' => ['required','date'], 'status' => ['required','in:draft,ordered,cancelled'], 'notes' => ['nullable','string']]));
    ...
    public function receive(PurchaseOrder $purchaseOrder): JsonResponse
    {
        ...
        DB::transaction(function () use ($purchaseOrder) {
            foreach ($purchaseOrder->items as $item) {
                StockTransaction::create(['product_id' => $item->product_id, 'transaction_type' => 'in', 'quantity' => $item->quantity, 'reference_type' => 'purchase', 'reference_id' => $purchaseOrder->id, 'employee_id' => $purchaseOrder->employee_id, 'notes' => 'Receive PO '.$purchaseOrder->po_number, 'transaction_date' => now(), 'created_at' => now()]);
            }
